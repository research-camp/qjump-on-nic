...
static void insert_packet(struct sk_buff *skb, int pid) {
    if (queue_index < MAX_QUEUE_SIZE) {
        packet_queue[queue_index].skb = skb;
        packet_queue[queue_index].pid = pid;
        queue_index++;
    } else {
...
static int compare(const void *a, const void *b) {
    return ((struct packet_info *)a)->pid - ((struct packet_info *)b)->pid;
}
